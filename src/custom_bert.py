...
def cached_path(url_or_filename, cache_dir=None):
    """
    获取给定的路径或 URL 并将其缓存到本地，如果已缓存则直接返回缓存文件的路径。

    Args:
        url_or_filename (str): URL 或本地文件路径。
        cache_dir (str, optional): 缓存目录，默认为用户主目录下的`.cache`目录。

    Returns:
        str: 本地文件路径。

    Raises:
        FileNotFoundError: 如果无法找到本地文件或从 URL 下载失败。
    """
    if cache_dir is None:
        cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "pretrained_models")
    ...
    if parsed.scheme in ("http", "https"):
        ...
        # 下载文件
        with requests.get(url_or_filename, stream=True) as response:
            if response.status_code != 200:
                raise FileNotFoundError(
                    f"Failed to fetch {url_or_filename} with status code {response.status_code}"
                )
    ...
    raise FileNotFoundError(f"{url_or_filename} is not a valid file or URL.")
...
class BertIntermediate(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.dense(hidden_states)
        hidden_states = self.intermediate_act_fn(
            hidden_states
        )  # [batch, max_len, nslot]
    ...
    def forward(
        ...
        if imp_op == "return":
            last_hidden, ffn_weights, imp_weights = self.bert(
                ...
                imp_pos=imp_pos,
                imp_op=imp_op,
            )  # (batch, max_len, hidden_size), (batch, max_len, ffn_size), (n_imp_pos)
        else:
