...
class CustomQwen2ForCausalLM(Qwen2ForCausalLM):
    ...
    def forward(self, target_token_idx, *args, **kwargs):
        ...
        def hook_fn(module, input):
            self._intermediate_activations.append(input[0][:, target_token_idx, :])

        hooks = []
        for layer in self.model.layers:
            hooks.append(layer.mlp.down_proj.register_forward_pre_hook(hook_fn))
    ...
    def _forward_with_partitioning(self, target_token_idx, times, predicted_label):
        ...
        for layer_idx in range(num_layers):
            for param in self.model.layers[layer_idx].mlp.parameters():
                param.requires_grad = True
    ...
    def _create_layer_hook(self, target_token_idx, activations, target):
        def hook_fn(module, input):
            _input = input[0].clone()
            # 直接替换当前batch所有样本的对应位置
            _input[:, target_token_idx] = activations
            return (_input,)

        return target.register_forward_pre_hook(hook_fn)
